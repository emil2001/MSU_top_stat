  ...
  class StackDrawer{
    public : 
      ...
      TCanvas * GetCanvas(const char * name, int width = 640 * 2, int height = 480 * 2){
        ...
        std::vector <int> colors = mRoot::getNiceColors(100);
        int color = 0;
        int signal_color = 2;
        ...
        pad_main->Draw();
        pad_main->cd();
        hs->Draw("hist am");//"hist");
        std::ofstream out("/home/emlg/CLionProjects/test1/FCNC_tqg_stat/2021_UL17_deep3/sm/def/postfit_hists/posthists_saved.txt");

        hs->SavePrimitive(out);
